...
export default function Step1({ toggleTab, toggle, option, addSubscriberData, subscriberData, handleChange, formData, setSubscriberData, setBackBtnState }) {
    ...
    return (
        <>
            <form onSubmit={submitData}>
                ...
                <hr />
                {formData?.already_subscriber ? <div className='row'>
                    <div className='col-md-6 col-sm-6 col-12'>
                    ...
                    <div className='col-md-6 col-sm-6 col-12 mt-3'>
                        ...
                        <input type='text' className='form-control' required placeholder='' value={subscriberData?.userName} disabled />
                    </div>
                    <div className='col-md-6 col-sm-6 col-12 mt-3'>
                        <label className='form-label'>Phone Number</label>
                        <input type='text' className='form-control' required placeholder='' value={subscriberData?.mobile_number} disabled />
                    </div>
                </div> : <div className='row'>
                    <div className='col-12'>
                        <label className='form-label'>Full Name <span className='text-danger'>*</span></label>
                        <input type='text' className='form-control' required placeholder='Enter full name' value={formData?.name} name='name' onChange={handleChange} />
                    </div>
                    <div className='col-md-6 col-sm-6 col-12 mt-3'>
                        <label className='form-label'>Phone Number <span className='text-danger'>*</span></label>
                        <input type='number' className='form-control' placeholder='Enter phone no.' required value={formData?.mobile_number} name='mobile_number' onInput={(e) => (e.target.value = e.target.value.slice(0, 10))} onChange={(e) => {
                            handleChange(e)
                            setPhoneErr(false)
                        }} />
                        {phoneErr ? <div className='text-danger'>Invalid phone number</div> : ''}
                    ...
                    <div className='col-md-6 col-sm-6 col-12 mt-3'>
                        ...
                        <input type='email' className='form-control' required placeholder='Enter email id' value={formData?.email} name='email' onChange={handleChange} />
                    </div>
                </div>}
                <div className='w-100 d-flex justify-content-end mt-5'>
                    <button className='btn text-primary' type='button' onClick={toggle}>Cancel</button>
                    <button className='btn btn-primary' type='submit'>Next <FaArrowRight /></button>
                </div>
            </form>
        </>
    )
