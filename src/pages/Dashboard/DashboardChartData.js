import { dashboaredActiveUser, lastMonthRevenueData, toTalSubcriberData, totalRevenueData } from "../../service/admin";

export async function activeUserfunction() {
  let res = await dashboaredActiveUser("664645a80522c3d5bc318cd2");
  ...
  return {
    ...
    userDistribution: {
      labels: resdata?.distribution ? resdata?.distribution?.map(e => e.role) : [],
      dataUnit: "BTC",
      legend: false,
      datasets: [
        {
...
export async function lastRevenueofMonth() {
  let res = await lastMonthRevenueData("664645a80522c3d5bc318cd2");
  let resdata = res.data.data;
  return {
    labels: resdata && resdata?.length !== 0 ? resdata?.map(e => e.plan) : [],
    dataUnit: "People",
    datasets: [
      {
        label: "Revenue",
...
export async function dataofSubcription() {
  let res = await toTalSubcriberData("664645a80522c3d5bc318cd2");
  let resdata = res.data.data;
  return {
    labels: resdata && resdata.length ? resdata?.map(e => e.plan) : [ ],
    dataUnit: "People",
    datasets: [
      {
        label: "Total",
...
export async function totalDataRevenue(year) {
  let res = await totalRevenueData({ year, id: "664645a80522c3d5bc318cd2" });
  let resdata = res.data.data;
  //
  return {
