...
import { addPropsal, nextSeq } from "../../../service/admin";
...
export default function ProposalPreview({
  ...
  const adddProposalSubmit = async e => {
    ...
    let payloadDatas = {
      ...
      grand_total: getTotalAmount() - (getTotalAmount() / 100) * Number(discountPer).toFixed(2),
  ...
  const discountedAmount = getTotalAmount() - (getTotalAmount() / 100) * Number(discountPer);
  return (
    <>
              ...
              <div style={styleSheet.maincontainer} ref={inVoiceRef1} id="print" className=" ">
                ...
                <div className="d-flex justify-content-between mt-3">
                  <div className="flex-direction-column f-18">
                    ...
                    <div className="mt-1">
                      <span className="fw-600">Shipping Address</span> : {getDataById?.installation_address?.pin_code},
                      {getDataById?.installation_address?.flat_number}, {getDataById?.installation_address?.city},{" "}
                      {getDataById?.installation_address?.state}
                    </div>
                    <div className="mt-1">
                      <span className="fw-600">Billing Address</span> : {getDataById?.billing_address?.pin_code},
              ...
              <div style={styleSheet.maincontainer} ref={inVoiceRef2} id="print2" className=" ">
                ...
                <div className="mt-3">
                    ...
                    <tbody>
                      <tr>
                        <td>
                          Rs.
                          {addTaxesinPrice(
                            Number(planInfo?.amount) - (Number(planInfo?.amount) / 100) * Number(planInfo?.discount),
                            planInfo?.cgst,
                            planInfo?.sgst
                          ).toFixed(2)}
                        </td>
                        <td>
                          ...
                          {inventeryInfo
                            .reduce((accumulator, object) => {
                              let gst =
                                Number(object?.price) * (Number(object?.cgst || 0) / 100) +
                                Number(object?.price) * (Number(object?.sgst || 0) / 100);

                              return accumulator + Number(object.total) + gst;
                            }, 0)
                            .toFixed(2)}
                        </td>
                        <td>
                  ...
                  <div className="mt-2">
                    ...
                    <div style={{ fontSize: "0.9em" }}>({getAmountInWords(discountedAmount)})</div>
                  </div>
                </div>
