...
import { addBandWidths } from "../../service/admin";
import Loader from "../../components/commonComponent/loader/Loader";
import { userId, userInfo } from "../../assets/userLoginInfo";
import { success } from "../../Store/Slices/SnackbarSlice";
import { useDispatch } from "react-redux";
import { getAllBngData } from "../../service/admin";
import Select from "react-dropdown-select";

export const AddBandwidth = ({ modal, setModal, getAllBandWidthFunction }) => {
  ...
  const handleBngChange = e => {
    const selectedBngId = e.target.value;
    ...
    if (maxDownloadKbps > minDownloadKbps && maxUploadKbps > minUploadKbps) {
      ...
      try {
        ...
        dispatch(
          success({
            show: true,
            msg: "Bandwidth Created successfully",
            severity: "success"
          })
        );
      } catch (err) {
  ...
  //     // Prepare the payload with BNG data
  //     let payloadData = {
  //       ...formData,
  //       bng_attribute_id: selectedBng,
  //       bng_name: getAllDataBng.find(bng => bng._id === selectedBng)?.bng_name || "",
    ...
    <>
      ...
      <Modal isOpen={modal} size="xl">
        <ModalHeader toggle={toggle} className="d flex align-items-center justify-content-between pop_up">
          <span className={`head_min  bandWidthHeader`}>Create New Bandwidth Template</span>
        </ModalHeader>
        <ModalBody>
              ...
              <div className="row mt-4">
                <div className="col-12 col-md-6 col-lg-6 mb-2">
                  <FormGroup>
                    ...
                    <Input type="select" name="bng" id="bng" value={selectedBng} onChange={handleBngChange} required>
                      <option value="" disabled>
                        Please Select Bng
                      </option>
                      {getAllDataBng && Array.isArray(getAllDataBng) && getAllDataBng.length > 0 ? (
