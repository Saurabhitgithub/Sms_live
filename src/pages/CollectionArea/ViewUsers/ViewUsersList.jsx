...
import { getallApprovalCollectionInfo } from "../../../service/admin";
...
export default function ViewUsersList() {
  ...
  const getcollectioninfo = async () => {
    setLoader(true);
    try {
      let response = await getallApprovalCollectionInfo().then(res => {
        return res.data.data;
      });
      
      let exportinfo = response?.map(res => {
        return {
          Area: res?.areaInfo[0]?.name,
    ...
    try {
      ...
      if (text && text.length !== 0) {
        ...
        let searchTable = allTableData.filter(res => {
          return (
            (res?.areaInfo[0]?.name && res?.areaInfo[0]?.name.toLowerCase().includes(lowerCaseText)) ||
        ...
        let searchExport = allexportData.filter(res => {
          return (
            (res["Area"] && res["Area"].toLowerCase().includes(lowerCaseText)) ||
    ...
    <>
      ...
      <div style={{ width: "1000px" }} className="">
        ...
        {loader ? (
          ...
          <>
          {permissionAccess && leadPermission.includes("Viewing Users Tab") ? (
            <>
               <div className="card_container p-md-4 p-sm-3 p-3">
              <div className="topContainer">
                <div className="f-28">View Users</div>
                <div className="d-flex align-items-center mt-md-0 mt-sm-0 mt-1">
                  <div className="ml-3">
                    {/* <ExportCsv filName={"View Users"} exportData={exportData} /> */}
              <div className="dropdown_logs ">
                    ...
                    <Dropdown isOpen={dropdownOpen} toggle={toggleDropdown}>
                      ...
                      <DropdownMenu>
                        <DropdownItem>
                          <ExportCsv exportData={exportData} filName={"View Users"} />
                        </DropdownItem>
                        <DropdownItem>
                          {" "}
